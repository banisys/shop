...
<header class="clearfix" style="width: 600px;margin: 0 auto ">
    ...
    <div id="project">
        ...
        <div><span>استان:</span> {{$order->state}}</div>
        <div><span>شهر:</span> {{$order->city}}</div>
        <div><span>کد پستی:</span> {{$order->postcode}}</div>
        <div><span>آدرس:</span> {{$order->address}}</div>
    </div>
</header>
<main>
    <table style="border: 1px solid black;width: 600px;margin: 0 auto ">
        ...
        <tbody>
        <?php  $sum = 0 ?>
        @foreach($order->order_pro as $product)
            <tr>
                <td style="border: 1px solid black">{{$product->product->name}}</td>
                <td style="border: 1px solid black"> {{$product->num}}</td>
                <td style="border: 1px solid black">{{$product->product->price}}</td>
                <td style="border: 1px solid black">{{$product->product->price*$product->num}}</td>
                <?php $sum += $product->product->price * $product->num  ?>
            </tr>
        @endforeach
        </tbody>
    </table>
